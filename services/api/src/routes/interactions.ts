...
router.post('/bookmarks', async (req: AuthRequest, res) => {
  ...
  if (!readerId || !editionId) return res.status(400).json({ error: 'readerId_and_editionId_required' });
  const pool = getPool();
  ...
  try {
    ...
    const [ins]: any = await conn.query('INSERT INTO bookmarks (readerId, editionId, pageNumber, createdAt) VALUES (?, ?, ?, NOW())', [readerId, editionId, pageNumber || null]);
...
router.delete('/bookmarks/:id', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const [rows]: any = await conn.query('SELECT b.id FROM bookmarks b JOIN readers r ON b.readerId = r.id WHERE b.id = ? AND r.userId = ? LIMIT 1', [id, userId]);
...
router.post('/highlights', async (req: AuthRequest, res) => {
  ...
  if (!userId) return res.status(401).json({ error: 'unauthenticated' });
  if (!readerId || !editionId || !text) return res.status(400).json({ error: 'readerId_editionId_text_required' });
  const pool = getPool();
  const conn = await pool.getConnection();
  try {
    const [rRows]: any = await conn.query('SELECT id FROM readers WHERE id = ? AND userId = ? LIMIT 1', [readerId, userId]);
    if (!rRows[0]) return res.status(403).json({ error: 'forbidden' });
    const [ins]: any = await conn.query('INSERT INTO highlights (readerId, editionId, pageNumber, text, color, createdAt) VALUES (?, ?, ?, ?, ?, NOW())', [
      readerId,
      editionId,
      pageNumber || null,
      text,
...
router.delete('/highlights/:id', async (req: AuthRequest, res) => {
  ...
  const conn = await pool.getConnection();
  try {
    const [rows]: any = await conn.query('SELECT h.id FROM highlights h JOIN readers r ON h.readerId = r.id WHERE h.id = ? AND r.userId = ? LIMIT 1', [id, userId]);
    if (!rows[0]) return res.status(404).json({ error: 'not_found' });
...
router.post('/notes', async (req: AuthRequest, res) => {
  ...
  if (!userId) return res.status(401).json({ error: 'unauthenticated' });
  if (!readerId || !editionId || !content) return res.status(400).json({ error: 'readerId_editionId_content_required' });
  const pool = getPool();
  const conn = await pool.getConnection();
  try {
    const [rRows]: any = await conn.query('SELECT id FROM readers WHERE id = ? AND userId = ? LIMIT 1', [readerId, userId]);
    if (!rRows[0]) return res.status(403).json({ error: 'forbidden' });
    const [ins]: any = await conn.query('INSERT INTO notes (readerId, editionId, pageNumber, content, createdAt) VALUES (?, ?, ?, ?, NOW())', [
      readerId,
      editionId,
      pageNumber || null,
      content
...
router.put('/notes/:id', async (req: AuthRequest, res) => {
  ...
  const conn = await pool.getConnection();
  try {
    const [rows]: any = await conn.query('SELECT n.id FROM notes n JOIN readers r ON n.readerId = r.id WHERE n.id = ? AND r.userId = ? LIMIT 1', [id, userId]);
    if (!rows[0]) return res.status(404).json({ error: 'not_found' });
